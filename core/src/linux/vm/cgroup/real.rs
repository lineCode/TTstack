...
const CGROUP_ROOT_PATH: &str = "/tmp/.ppcgroup";

// 管理进程的 Cgroup 路径
const CGROUP_ADMIN_PATH: &str = "/tmp/.ppcgroup/ppadmin";
...
/// 清理 Vm 进程[组]
pub(in crate::linux) fn kill_vm(id: VmId) -> Result<()> {
    get_proc_meta_path(id)
        .c(d!())
...
fn kill_cgrp(cgpath: PathBuf) -> Result<()> {
    fs::read(&cgpath)
        ...
        .and_then(|dir| fs::remove_dir(dir).c(d!()))
}
