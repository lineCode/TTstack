...
macro_rules! gen_sock {
    ($kind: tt, $addr_family: tt, $sock_type: tt, $timeout: expr) => {{
        ...
        .and_then(|sock| unsafe {
            let s = $kind::from_raw_fd(sock);
            s.set_read_timeout($timeout.map(Duration::from_secs))
                .c(d!())?;
            Ok($kind::into_raw_fd(s))
        })
        .map(FileHdr)
    }};
}
...
/// 创建 UDP 套接字
#[inline(always)]
pub fn gen_udp_sock(timeout: Option<u64>) -> Result<FileHdr> {
    gen_sock!(UdpSocket, Inet, Datagram, timeout)
}

/// 创建 TCP 套接字
#[inline(always)]
pub fn gen_tcp_sock(timeout: Option<u64>) -> Result<FileHdr> {
    gen_sock!(TcpStream, Inet, Stream, timeout)
}
