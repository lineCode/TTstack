...
fn exec_normal_success() {
    let resp = pnk!(client::req_exec(UDP_SERV_ADDR, "uname", None));
...
fn exec_normal_fail() {
    let resp =
        pnk!(client::req_exec(UDP_SERV_ADDR, "ls /a;lfjkal;hjkf", None));
...
fn exec_timeout() {
    let resp = client::req_exec(UDP_SERV_ADDR, "sleep 2", Some(1));
    assert!(resp.is_err());
}
