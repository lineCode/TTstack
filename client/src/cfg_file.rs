...
use std::{env, fs, net::SocketAddr, path, process};

lazy_static! {
    /// 客户端配置路径
    pub static ref CFG_PATH: String = format!("{}/.pp",pnk!(env::var("HOME")));
    /// 客户端配置文件
    pub static ref CFG_FILE: String = format!("{}/pp.json", CFG_PATH.as_str());
}
...
lazy_static! {
    ...
    pub static ref SSH_VM_KEY: String =
        format!("{}/pp_rsa", SSH_CFG_DIR.as_str());
}
...
fn set_sshenv() -> Result<()> {
    fs::create_dir_all(SSH_CFG_DIR.as_str())
        ...
        .and_then(|_| {
            cmd_exec("chmod", &["0400", SSH_VM_KEY.as_str()]).c(d!())
        })
}
