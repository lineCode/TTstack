        ...
        .subcommands(vct![
            ...
            SubCommand::with_name("env").subcommands(vct![
                SubCommand::with_name("add")
                    ...
                    .arg_from_usage("-t, --vm-type=[TYPE] 'VM 类型, 暂时不支持自定义.'")
                ...
                SubCommand::with_name("update")
                    ...
                    .arg_from_usage("--I-am-a-crazy-monkey-AND-I-am-crazy-now '指定任意生命周期.'")
                    .arg_from_usage("-l, --life-time=[TIME] '新的生命周期.'")
                    .arg_from_usage("-k, --kick-vm=[OS_PREFIX]... '待剔除的系统名称前缀.'"),
...
fn env_add<'a>(m: &'a ArgMatches<'a>) -> Result<EnvAdd<'a>> {
    match (
        ...
        (
            ...
            dup_each: option_num_parse!(dup_each, 0, u8),
        }),
        _ => Err(eg!()),
    }
}
...
fn env_update<'a>(m: &'a ArgMatches<'a>) -> Result<EnvUpdate<'a>> {
    match (
        ...
        (_, _, None) => Err(eg!("没有指定 ENV")),
        (life_time, os_prefix, Some(env_set)) => Ok(EnvUpdate {
            is_fucker: m
                .is_present("--I-am-a-crazy-monkey-AND-I-am-crazy-now"),
            ...
            os_prefix: os_prefix
                .map(|osp| osp.map(|o| o.to_owned()).collect()),
            env_set: env_set.collect(),
        }),
    }
}
...
fn env_get<'a>(m: &'a ArgMatches<'a>) -> Result<EnvGet<'a>> {
    ...
    match (
        ...
        (None, _, _) => Err(eg!("没有指定 --file-path")),
        (_, _, None) => Err(eg!("没有指定 ENV")),
        (Some(file_path), timeout, Some(env_set)) => Ok(EnvGet {
...
fn env_push<'a>(m: &'a ArgMatches<'a>) -> Result<EnvPush<'a>> {
    ...
    match (
        ...
        (None, _, _) => Err(eg!("没有指定 --file-path")),
        (_, _, None) => Err(eg!("没有指定 ENV")),
        (Some(file_path), timeout, Some(env_set)) => Ok(EnvPush {
            use_ssh: m.is_present("use-ssh"),
...
fn env_run<'a>(m: &'a ArgMatches<'a>) -> Result<EnvRun<'a>> {
    ...
    match (
        ...
        (None, None, _, _) => Err(eg!("没有指定 --cmd 或 --script")),
        (_, _, _, None) => Err(eg!("没有指定 ENV")),
        (cmd, Some(script), timeout, Some(env_set)) => Ok(EnvRun {
            use_ssh: m.is_present("use-ssh"),
            cmd: cmd.unwrap_or(""),
        ...
        (Some(cmd), None, timeout, Some(env_set)) => Ok(EnvRun {
            use_ssh: m.is_present("use-ssh"),
            cmd,
            script: "",
            time_out: timeout.unwrap_or(TIMEOUT).parse::<u64>().c(d!())?,
